...
#define JSONXX_ASSERT(...) do { if( jsonxx::Assertions ) \
  jsonxx::assertion(__FILE__,__LINE__,#__VA_ARGS__,jsonxx::const_condition(bool(__VA_ARGS__))); } while(0)


namespace jsonxx {

	template<typename T> const T& const_condition(const T& t) { return t; }
	// Settings
	enum Settings {
		// constants
		Enabled = true,
