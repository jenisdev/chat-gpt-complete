...
namespace RESTAPI {
	...
	public:
		...
		ihandle RequestWithBuffer(const wchar_t* url, const wchar_t* verb = "POST", std::initializer_list<wstring> hdrs = {}, const char * d = 0, size_t sz = 0, std::function<HRESULT(size_t sent, size_t tot, void*)> fx = nullptr, void* lp = 0, bool Once = true, DWORD ExtraSecurityFlags = 0)
		{
			memory_data_provider m(d, sz);
			return Request2(url, m, Once, verb, hdrs, fx, lp, 0, 0, 0, 0, ExtraSecurityFlags);
		}

		ihandle RequestWithFile(const wchar_t* url, const wchar_t* verb = "POST", std::initializer_list<wstring> hdrs = {}, HANDLE hX = INVALID_HANDLE_VALUE, std::function<HRESULT(size_t sent, size_t tot, void*)> fx = nullptr, void* lp = 0, DWORD ExtraSecurityFlags = 0)
		{
			file_data_provider m(hX);
			return Request2(url, m, true, verb, hdrs, fx, lp, 0, 0, 0, 0, ExtraSecurityFlags);
		}

		ihandle Request2(const wchar_t* url, data_provider& dp, bool Once = true, const wchar_t* verb = "POST", std::initializer_list<wstring> hdrs = {}, std::function<HRESULT(size_t sent, size_t tot, void*)> fx = nullptr, void* lp = 0, const char* extradata1 = 0, DWORD extradatasize1 = 0, const char* extradata2 = 0, DWORD extradatasize2 = 0, DWORD ExtraSecurityFlags = 0)
		{
			if (!url)
				return 0;
			wstring nurl;
			ihandle hI3;
			if (_wcsnicmp(url, "http://", 7) == 0 || _wcsnicmp(url, "https://", 8) == 0)
			{
				Disconnect();
				hI = InternetOpen(Agent.c_str(), INTERNET_OPEN_TYPE_DIRECT, 0, 0, 0);
				if (_wcsicmp(verb, "GET") == 0)
				{
